...
Let's examine the fatal police shooting data and see if the data is poisson distributed.
First let's divide the total number of shooting by the total population. Then, we'll generate the expected rate for each state based on our expected rate $\lambda$ and the state population.
```{r}
#(lambda <- sum(p_df$gunshot_tally)/sum(p_df$total_pop/1000))
(lambda <- sum(p_df$gunshot_tally)/sum(p_df$total_pop)) # mean number of fatal shootings per 1000000
# calculate probability of observing each number of shootings
# then multiply by total ... number of people?

t <- dpois(c(0:100), lambda, log = FALSE)
expected <- c(1:50)
for (i in c(1:50))
{
  expected[i] <- lambda*p_df$total_pop[i] #sum(t*p_df$total_pop[i])/100000
}
```

Now use Pearson's goodness of fit chi-square to see if a Poisson model is appropriate.

[A helpful link](https://chemicalstatistician.wordpress.com/2013/04/14/checking-the-goodness-of-fit-of-the-poisson-distribution-for-alpha-decay-by-americium-241/)
```{r}
...
```
Test negative binomial and poisson
```{r}
library(MASS)
