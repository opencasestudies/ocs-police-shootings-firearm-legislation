...
violentcrime[11, ]
```
We need get all rows where `X__1 == Rate per 100,000 inhabitants`. However, as we can see above, the value for `State` in these rows is `<NA>`, so we need to fill that value with the state name that is listed in a previous row.
...
NOTE TO SELF: ROUND NUMBERS PRINTED
```{r}
for(col in c(2,3,4,5,7,8,13,15,16)){
    corr <- cor.test(x=p_df[,12], y=p_df[,col], method = 'spearman',exact=FALSE)
    print(paste0(colnames(p_df)[col], " correlation with fatal shooting rate: " , corr$estimate))
  }
```
  ...
  for(col2 in c(5,7,8,15,16)){
    ...
    corr <- cor.test(x=p_df[,col1], y=p_df[,col2], method = 'spearman',exact=FALSE)
      if(corr$estimate>= 0.3 | corr$estimate <= -.3){
        print(paste0(colnames(p_df)[col1], " correlation with ", colnames(p_df)[col2], ": ",corr$estimate))
      }
    }
  }
}
...
med.fit <- lm(ownership ~ brady_scores + age + violent_crime, data = p_df)
out.fit <- glm(gunshot_tally ~ ownership + brady_scores + age  + violent_crime + offset(log(total_pop)), 
                family="poisson", data = p_df)
