...
#summary(lm1 <- lm(gunshot_rate ~ brady_scores, data = p_df))
#ggplot(p_df, aes(x = brady_scores)) + geom_point(aes(y = gunshot_rate, color = "data")) + geom_line(aes(y = lm1$fitted.values , color = "fitted")) + ggtitle("Fatal Police Shootings per 1,000,000 Simple Linear Regression")

summary(m1 <- glm(gunshot_tally ~  brady_scores + offset(log(total_pop)), family="poisson", data = p_df))

resultY <- exp(m1$coefficients[1])*exp(m1$coefficients[2]*p_df$brady_scores)*1000000/2
dat <- as.data.frame(resultY)
dat$X <- p_df$brady_scores
dat$trueY <- p_df$gunshot_tally/p_df$total_pop*1000000/2

ggplot(dat, aes(x = X)) + geom_point(aes(y = trueY, color = "data")) + geom_line(aes(y = resultY , color = "fitted")) + ggtitle("Fatal Police Shootings Poisson Regression") + ylab("Annualized Rate of Fatal Police Shootings per 1 000 000") + xlab("Firearm Legislative Strength Score")
```
...
```{r}
# mv for multivariate
m2 <- glm(gunshot_tally ~  brady_scores + white + black + hispanic + male + age + offset(log(total_pop)), family="poisson", data = p_df)
summary(m2)
m2_Xvars <- p_df %>%
